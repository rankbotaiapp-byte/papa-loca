...
import type { BusinessProfile, HaloTheme } from "@/lib/axiom/types";
  ...
  return (
    <main className="relative pb-8">
      ...
      <div className="relative flex flex-col gap-5 px-4 pt-4">
        ...
        {business.niche === "food_truck" || business.locationNote ? (
          <Card className="p-4">
            <p className="text-xs font-medium uppercase tracking-widest text-muted-foreground">
              {copy.locationLabel}
            </p>
            <p className="mt-2 font-display text-lg font-medium tracking-display">
              {business.locationName || "Location posts daily"}
            </p>
        ...
        {business.posts[0] ? (
          <Card>
            ...
            <p className="mt-2 text-sm leading-normal text-foreground">{business.posts[0].body}</p>
            <p className="mt-2 text-xs text-subtle">{relativeFrom(business.posts[0].createdAt)}</p>
          </Card>
        ) : null}

        <CrewBlocks slug={slug} />
              ...
              {business.offerings.map((offering) => (
                <div key={offering.id} className="overflow-hidden rounded-lg bg-card/90">
                  ...
                  <div className="p-3">
                    ...
                    <p className="mt-2 text-xs text-subtle">{offering.durationMin}m</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        {business.posts.length > 1 ? (
          <section>
            <h2 className="text-xs font-medium uppercase tracking-widest text-muted-foreground">Posts</h2>
            <div className="mt-3 flex flex-col gap-3">
              {business.posts.slice(1).map((post) => (
                <Card key={post.id} className="p-3">
                  {post.image ? (
                    <img src={post.image} alt="" className="mb-3 h-32 w-full rounded-md object-cover" />
                  ) : null}
                  <p className="text-sm">{post.body}</p>
                  <p className="mt-2 text-xs text-subtle">{formatDayTime(post.createdAt)}</p>
                </Card>
              ))}
            </div>
          </section>
...
function AskAxiom({ slug }: { slug: string }) {
  ...
  return (
    <Card className="flex flex-col gap-2 p-4">
      ...
      <p className="text-xs text-subtle">Answers only from this shop — services, crew, hours, place, posts.</p>
      <form
      ...
      >
        <Input
          ref={askRef}
          name="question"
          placeholder="Skin fade price, who’s in tonight…"
          autoComplete="off"
          maxLength={240}
        />
